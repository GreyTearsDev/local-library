...
exports.book_create_post = [
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      // Data is valid
      // Save book and redirect to its page
      await book.save();
...
exports.book_delete_get = asyncHandler(async (req, res, next) => {
  ...
  if (!book) {
    // the book does not exist. Redirect the user to the catalog containing all books
...
exports.book_delete_post = asyncHandler(async (req, res, next) => {
  const [book, allBookInstances] = await Promise.all([
    Book.findById(req.params.id).exec(),
    BookInstance.find({ book: req.params.body }).exec(),
  ]);

  if (allBookInstances.length > 0) {
    // There are instances of this book. Render the same as on the GET request
    res.render("book_delete", {
      title: "Delete Book",
      book: book,
      book_instance_list: allBookInstances,
    ...
    // No instances of the book were found. It is safe to delete it
